...
That failure mode should no longer be the default operating model. The current manifests pull a versioned GHCR image such as `ghcr.io/m-ryan-nugent/cluster-dashboard:v1.0.0` with `imagePullPolicy: IfNotPresent` and do not pin the pod to `pi-worker-1`.
